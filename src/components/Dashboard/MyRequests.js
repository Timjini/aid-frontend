...
export default function MyRequests(){
   ...
   // get my requests with headers
    const getRequests = async (e) => {
        try {
          const response = await authenticationApi
            .myRequests(`${API_REQUESTS}/my_requests`, setAuthHeaders())            
                ...
                <Link to={`/requests/${request.id}`}>
                  ...
                  <TabPanel>
                    {requests.map((request) => {
                      if (request.situation === 'Fulfilled'){
                        return (
                          <Flex
                          key={request.id}
                          boxShadow={'lg'}
